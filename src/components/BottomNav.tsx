...
import { ActiveChalkCircle, BlobStamp } from "@/components/nav-shapes";
...
export function BottomNav() {
  ...
  return (
    <nav
      className={`pointer-events-none fixed inset-x-0 bottom-0 z-40 flex justify-center px-4 pb-[max(env(safe-area-inset-bottom),0.75rem)] transition-transform duration-300 lg:hidden ${
        ...
        {items.map((it) => {
          ...
          return (
            ...
            >
              ...
              {active && (
                <span className="font-chalk text-[13px] leading-none" style={{ color: "#EFE6D2" }}>
                  {it.label}
                </span>
              )}
            </Link>
